...
SpriteSheetClass = Class.extend({
	...
	parseAtlasDefinition: function (atlasJSON) {
        ...
        // variable.
        //
        var jsondata = JSON.parse(atlasJSON);

	for(var key in jsondata.frames) {

	var current = jsondata.frames[key];

        var cx = -current.frame.w * .5 ;
        var cy = -current.frame.h * .5;

	this.defSprite(key, current.frame.x, current.frame.y, current.frame.w, current.frame.h, cx, cy);
		}
